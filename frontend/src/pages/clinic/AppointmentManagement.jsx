...
export default function AppointmentManagement() {
    ...
    // --- MODAL STATE'LERİ ---
    const [selectedPet, setSelectedPet] = useState(null); // Küçük modal için
    ...
    // 2. İptal İşlemleri
    const handleCancelClick = (apptId) => setCancelModal({ show: true, apptId });
    ...
    // 3. Küçük Modal İçin Pet Verisi Çek
    const handlePetClick = async (appt) => {
        ...
        try {
            ...
            if (data.photoUrl && !data.photoUrl.startsWith("http")) {
                data.photoUrl = `http://localhost:8080/uploads/pets/${data.photoUrl}`;
            }

            setSelectedPet({ ...data, owner: appt.petOwnerName, loading: false });
        } catch (error) {
            console.error(error);
            setSelectedPet(null);
            alert("Pet detay hatası.");
        }
    };
    ...
    // =================================================================================
    // RENDER: LİSTELEME EKRANI
    // =================================================================================
    return (
        <div>
            <h2 style={headerStyle}>📂 Randevu Yönetimi</h2>
            <p style={subHeaderStyle}>Geçmiş ve gelecek tüm randevuları filtreleyin ve yönetin.</p>
            ...
            <div style={searchBoxStyle}>
                ...
                <div style={{ display: "flex", alignItems: "flex-end" }}>
                    <HoverButton onClick={handleSearch} baseStyle={searchBtnBase} hoverStyle={searchBtnHover}>
                        🔍 Listele
                    </HoverButton>
                </div>
            </div>
            ...
            {loading ? (
                <div style={{ padding: 40, textAlign: "center" }}>⏳ Aranıyor...</div>
            ) : (
                <>
            ...
            {cancelModal.show && (
                <div style={modalOverlayStyle}>
                    <div style={modalBoxStyle}>
                        ...
                        <p>Randevuyu iptal etmek istiyor musunuz?</p>
...
const searchBoxStyle = { display: "flex", gap: "20px", background: "white", padding: "20px", borderRadius: "12px", border: "1px solid #e2e8f0", marginBottom: "30px", alignItems: "center" };
const inputGroup = { display: "flex", flexDirection: "column", gap: "5px" };
const labelStyle = { fontSize: "12px", fontWeight: "bold", color: "#64748b" };
const inputStyle = { padding: "10px", borderRadius: "6px", border: "1px solid #cbd5e1", width: "160px" };
const searchBtnBase = { padding: "10px 20px", background: "#3b82f6", color: "white", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: "bold", height: "40px" };
const searchBtnHover = { background: "#2563eb" };
const emptyStyle = { padding: "40px", textAlign: "center", background: "white", borderRadius: "10px", border: "1px dashed #cbd5e1", color: "#94a3b8" };
const tableContainer = { overflowX: "auto", background: "white", borderRadius: "12px", border: "1px solid #e2e8f0", overflow: "hidden" };
const tableStyle = { width: "100%", borderCollapse: "collapse" };
const thRowStyle = { background: "#f8fafc", borderBottom: "2px solid #e2e8f0" };
const thStyle = { padding: "15px", textAlign: "left", fontSize: "12px", fontWeight: "700", color: "#475569" };
...
const modalBoxStyle = { background: "white", padding: "25px", borderRadius: "15px", width: "380px", boxShadow: "0 20px 25px -5px rgba(0, 0, 0, 0.2)" };
const photoContainerStyle = { width: "60px", height: "60px", borderRadius: "50%", overflow: "hidden", background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center" };
const photoStyle = { width: "100%", height: "100%", objectFit: "cover" };
const gridContainer = { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", background: "#f8fafc", padding: "10px", borderRadius: "8px", marginTop: "10px" };
const detailLabel = { fontSize: "10px", fontWeight: "800", color: "#94a3b8" };
const closeXBtn = { background: "transparent", border: "none", cursor: "pointer", fontSize: "18px", color: "#94a3b8" };
const codeStyle = { background: "#f1f5f9", padding: "8px", borderRadius: "8px", fontFamily: "monospace", fontSize: "12px", display: "block", border: "1px solid #e2e8f0" };
...
const modalConfirmBtnBase = { padding: "8px 16px", background: "#ef4444", color: "white", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: "600" };
const modalConfirmBtnHover = { background: "#dc2626" };

// ✅ KOPYALA BUTONU
const copyBtnBase = { padding: "8px 12px", background: "white", color: "#64748b", border: "1px solid #cbd5e1", borderRadius: "8px", cursor: "pointer", fontWeight: "700", fontSize: "12px" };
